/**
 * Pocket Omaha — what every number on screen means.
 *
 * Three parts to each entry, because a definition alone would explain somebody
 * else's number. The figures here are the product of specific judgements --
 * which model runs for a bank, whose free cash flow is meaningless, what a
 * missing line item does to a score -- and those judgements are the part a
 * reader cannot get from a textbook. `computes` is where they are stated.
 *
 * Lives in core/ so the Android build and the PWA read the same text, and so
 * a scoring change and its explanation can be edited in one commit.
 */

/** Written once, because several entries need to say the same thing. */
const MISSING_RULE =
  'A measure the filings do not contain is reported as unavailable and dropped '
  + 'from its pillar, so it neither helps nor hurts. It is never assumed to be zero.';
...
/**
 * The twelve-point checklist and the pillar items name several of the same
...
/**
 * Every scoring item that appears on screen must have an entry, and no entry
 * may describe something that no longer exists. The test enforces both, so a
 * renamed metric fails the build rather than quietly losing its explanation.
 */
